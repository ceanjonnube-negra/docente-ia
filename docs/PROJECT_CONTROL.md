...
## C-005 — Infraestructura de base de datos completada (2026-08-03)

**Estado: la infraestructura de base de datos de Planeación quedó creada, y la de Seguimiento quedó reparada.** Herramienta usada: Supabase CLI, instalada como devDependency (`npm install supabase --save-dev`), autenticada por el usuario (`npx supabase login`, en una Terminal interactiva real — este entorno no tiene TTY para hacerlo directo) y vinculada al proyecto real (`npx supabase link --project-ref abdtrkdfobrkramerrrc`, el mismo proyecto `docente-ia` usado en todo este documento).

**Hallazgo importante durante la auditoría previa a la primera migración correctiva:** al verificar el esquema remoto real (sin `service_role`, por lectura vía PostgREST y, cuando fue necesario, consultas de solo lectura que el propio usuario ejecutó en el SQL Editor), se descubrió que **ya existía una tabla `planeaciones` parcial** — 7 de 13 columnas planeadas (`id`, `docente_id`, `grupo_id`, `fecha_inicio`, `fecha_fin`, `creado_en`, `actualizado_en`), sin `nombre`, `estado`, `version`, `ciclo_escolar_id`, `periodo_evaluacion_id` ni `proposito` — con una política RLS `FOR ALL` llamada `"docentes gestionan sus propias planeaciones"`, de origen desconocido (no proviene de ningún archivo de este repositorio). `planeacion_proyectos` no existía en absoluto. Además, se confirmó (primero por verificación visual del usuario en el panel de Supabase, luego contra `pg_class`) que **las 4 tablas de Seguimiento (`proyectos_seguimiento`, `hojas_evaluacion`, `seguimiento_resultados`, `seguimiento_versiones`) tenían RLS deshabilitada** desde que se aplicó `migrations/seguimiento_fase3.sql` — la migración creó las tablas completas, pero el bloque de RLS no quedó activo (evidencia real que contradijo una inferencia anterior de este mismo documento, que asumía —incorrectamente— que la atomicidad de la transacción garantizaba que todo el archivo se había aplicado por igual).

**3 migraciones aplicadas, todas vía `npx supabase db push`, todas verificadas antes con `--dry-run`:**

1. `migrations/planeacion_fase1.sql` — **nunca se ejecutó tal cual** (se descubrió el problema de la `planeaciones` parcial antes de correrla; hubiera sido un no-op silencioso sobre esa tabla por el `CREATE TABLE IF NOT EXISTS`). Queda en `migrations/` (raíz del repo) como referencia histórica del diseño original, sin commitear.
2. `supabase/migrations/20260802150000_reparacion_seguimiento_planeacion.sql` — completó las 6 columnas faltantes de `planeaciones` (sin tocar sus 7 columnas ni datos existentes), creó `planeacion_proyectos` completa, habilitó RLS en las 6 tablas (Planeación + Seguimiento), y creó 18 políticas SELECT/INSERT/UPDATE nuevas. **Efecto secundario no previsto:** sus `DROP POLICY IF EXISTS` usaban nombres nuevos que nunca coincidían con las políticas viejas reales (`proyectos_seguimiento_titular`, `hojas_evaluacion_titular`, `seguimiento_resultados_titular`, `seguimiento_versiones_titular`, `"docentes gestionan sus propias planeaciones"`) — cada `DROP` fue un no-op silencioso, dejando 23 políticas coexistiendo (18 nuevas + 5 viejas `FOR ALL`), detectado por el usuario vía `pg_policies` en el panel de Supabase.
3. `supabase/migrations/20260803090000_eliminar_politicas_all_heredadas.sql` — eliminó las 5 políticas heredadas por su nombre exacto real. **Verificado por el usuario directamente contra `pg_policies`:** 18 políticas totales, exactamente 1 SELECT + 1 INSERT + 1 UPDATE por tabla en las 6 tablas, 0 políticas ALL, 0 DELETE, 0 duplicadas. RLS habilitada en las 6.

**Historial remoto de migraciones (`supabase_migrations.schema_migrations`, antes vacío por completo — `seguimiento_fase3.sql` se había aplicado manualmente por fuera de este flujo):** ahora tiene exactamente 2 entradas, ambas con `local`/`remote` coincidentes — las 2 migraciones correctivas. `migrations/seguimiento_fase3.sql` y `migrations/planeacion_fase1.sql` siguen sin registrarse en este historial (viven fuera de `supabase/migrations/`, nunca se les aplicó `migration repair`) — pendiente de decisión futura, no urgente.

**Estado final de las 6 tablas (planeaciones, planeacion_proyectos, proyectos_seguimiento, hojas_evaluacion, seguimiento_resultados, seguimiento_versiones):**
- Estructura de columnas: completa en las 6 (verificado por lectura directa de cada columna vía PostgREST).
- RLS: habilitada en las 6 (verificado por el usuario contra `pg_class.relrowsecurity = true`; `rls_forzada` = false en las 6, aceptado como suficiente por ahora).
- Políticas: 18 totales, exactamente SELECT/INSERT/UPDATE por tabla, 0 ALL, 0 DELETE (verificado por el usuario contra `pg_policies`).
- Datos: ninguna fila eliminada ni modificada en ningún momento de todo este proceso — confirmado en cada paso, ninguna migración contuvo `DELETE`/`TRUNCATE`/`DROP TABLE`.
